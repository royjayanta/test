...
class CBRatingSystemWidget extends WP_Widget
{
    ...
    {
        parent::__construct('cbrp_top_rated', __('CBX Rating: Top Rated Posts', 'cbratingsystem'), array('description' => __('A widget to display top rated posts, pages or custom post types.', 'cbratingsystem')));
    }
...
//        echo '</pre>';
            ...
            <?php if (!empty($data)) : ?>
                <?php foreach ($data as $newdata) : ?>
                    <li>
                        ...
                        <span id="cbrp-top-rated<?php echo $widget_id_fresh . $newdata->post_id . '-' . $form_id; ?>"
                              style="margin: 0;"></span>
    ...
    function form($instance)
    {
        ...
        $instance = wp_parse_args((array)$instance, array('title' => __('Top Rated Posts', 'cbratingsystem'), 'order' => 'DESC', 'day' => 7, 'limit' => 10, 'type' => 'post', 'form_id' => $default));
...
class CBRatingSystemUserWidget extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct('cbrp_top_rated_user', __('CBX Rating: Top Rated User', 'cbratingsystem'), array('description' => __('A widget to display top rated user.', 'cbratingsystem')));
    }
            ...
            <?php if (!empty($data)) : ?>
                <?php foreach ($data as $newdata) : ?>
                    <li>
                        ...
                        ?>
                        <a href="<?php echo get_author_posts_url((int)$newdata['post_author']); ?>"><?php echo $author_info->display_name; ?></a> <?php echo $newdata['post_count']; ?> <?php _e('Posts', 'cbratingsystem'); ?>
                        <?php
    ...
    function form($instance)
    {
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id('user_id'); ?>"><?php _e("User Ids ('Blank or comma separate ids ) ", 'cbratingsystem') ?>
                :</label>
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id('limit'); ?>"><?php _e('No. To Display', 'cbratingsystem') ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('limit'); ?>" type="text"
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id('post_id'); ?>"><?php _e("Post Ids ('Blank or comma separate ids ) ", 'cbratingsystem') ?>
                :</label>
        ...
        <!--post type id -->
        <p>
            <label for="<?php echo $this->get_field_id('type'); ?>"><?php _e("Post Type", 'cbratingsystem') ?>
            </label>
        ...
        <!-- post type filter -->
        <p>
            <label for="<?php echo $this->get_field_id('post_filter'); ?>"><?php _e("Post Filter", 'cbratingsystem') ?>
                :</label>
        ...
        <!-- order by  type filter -->
        <p>
            <label for="<?php echo $this->get_field_id('firstorder'); ?>"><?php _e("First Sort By", 'cbratingsystem') ?>
                :</label>
