...
define('CB_RATINGSYSTEM_PLUGIN_VERSION', '3.7.4'); //need for checking verson
...
class CBRatingSystem
{
    ...
    public function __construct()
    {
        ...
        //CB_RATINGSYSTEM_PLUGIN_VERSION               = CB_RATINGSYSTEM_PLUGIN_VERSION;

        load_plugin_textdomain('cbratingsystem', false, dirname(plugin_basename(__FILE__)) . '/language/');
    ...
    public static function add_common_styles_scripts()
    {
        ...
        //wp_enqueue_style( 'jquery-uniform-style', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'css/uniform.aristo.min.css', array(), CB_RATINGSYSTEM_PLUGIN_VERSION );
        ...
        wp_register_style('jquery-selectize-style', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'css/jquery.selectize.css', array(), CB_RATINGSYSTEM_PLUGIN_VERSION);

        wp_register_script('cbrp-common-script', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/cbrating.common.script.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);
        //wp_enqueue_script( 'jquery-uniform', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/jquery.uniform.js', array( 'jquery' ), CB_RATINGSYSTEM_PLUGIN_VERSION );
        ...
        wp_register_script('jquery-selectize', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/jquery.selectize.min.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);

        //enqueue them

        wp_enqueue_style('cbrp-common-style');
        wp_enqueue_style('jquery-chosen-style');
        wp_enqueue_style('jquery-selectize-style');
    ...
    public static function get_language_strings()
    {
        ...
        $strings = array(
            ...
            'noRatedMsg'     => __('No Rating', 'cbratingsystem')
        );
    ...
    public static function load_scripts_and_styles()
    {
        ...
        /*
    if ( is_string( $theme_key ) && ! empty( $theme_key ) ) {
        wp_register_style( 'cbrp-extra-theme-style', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'css/extra.theme.style.css', array('cbrp-basic-review-style'), CB_RATINGSYSTEM_PLUGIN_VERSION );
        wp_enqueue_style('cbrp-extra-theme-style');
    }
    */
        ...
        wp_localize_script('cbrp-front-js', 'cbrating_prefix', self::get_language_strings());


        wp_register_script('cbrp-front-review-js', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/cbratingsystem.front.review.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION, true);
...
//        echo '</pre>';
        ...
        /*
    $output = '';
    $output .= '<ul class="cbrp-top-rated-wpanel" >';
        if (!empty($data)) :
            foreach ($data as $newdata) :
                $output .='<li>';
                    ...
                    $output .='<div id="cbrp-top-rated-sc-'.$newdata->post_id .'-'. $form_id.'" style="margin: 0;"></div>';
                    $output .= '<strong>' . (($newdata->per_post_rating_summary / 100) * 5) .'/5</strong> ';
                    $output .= '<a href="'.get_permalink($newdata->post_id).'">'.$newdata->post_title.'</a>';
                $output .= '</li>';
             endforeach;
         else:
            $output .='<li>'.__('No Results found', 'cbratingsystem').'</li>';
         endif;
    $output .='</ul>';
    */
        ob_start();
                ...
                <?php foreach ($data as $newdata) : ?>
                    <li>
                        <script type="text/javascript">
							...
							<span id="cbrp-top-rated-sc-<?php echo $newdata->post_id . '-' . $form_id; ?>"
                                  style="margin: 0;"></span>
    ...
    public static function standalone_singlePost_rating_summary($option, $show_title = 0, $show_form_id = 0, $show_text = 0, $show_star = 1, $show_single = 0, $text_label = '')
    {
        ...
        $rating_avg_hints = array(__('bad', 'cbratingsystem'), __('poor', 'cbratingsystem'), __('regular', 'cbratingsystem'), __('good', 'cbratingsystem'), __('gorgeous', 'cbratingsystem'));
        $rating_avg_hints = '["' . implode('","', $rating_avg_hints) . '"]';
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function ($) {
                        $('.cbrp-alone-rated<?php echo $average_rating['post_id']; ?>').raty({
                            ...
                            readOnly: true,
                            hintList: <?php echo $rating_avg_hints; ?>
                        });
                    });
                </script>
                <?php
                $show .= '  <span class="cbrp-alone-rated cbrp-alone-rated' . $average_rating['post_id'] . '" id="cbrp-alone-rated' . $average_rating['post_id'] . '" style="margin: 0;"></span>' . $text_label_html . '';
                $show .= '';
            }
            ...
            if (!empty($post_ids) && !empty($form_ids)) {
                ...
                foreach ($post_ids as $post_id) {
                    ...
                    if ($show_star == 1) {
                        ...
                        <?php
                        $show .= '  <span class="cbrp-alone-rated cbrp-alone-rated' . $average_rating['post_id'] . '" id="cbrp-alone-rated' . $post_id . '" style="margin: 0;"></span>' . $text_label_html;
                    }
                    $show .= '</div>';
                    //$position += 30;
                }
 ...
 * @return string
 */
function cbratingStandaloneReviews($form_id, $post_id, $theme_key = 'basic')
{
    ...
    $theme_key = ($theme_key == '') ? get_option('cbratingsystem_theme_key', 'basic') : 'basic';
