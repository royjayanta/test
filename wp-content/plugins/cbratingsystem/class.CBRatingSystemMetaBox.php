...
class CBRatingSystemMetaBox extends CBRatingSystemAdmin
{
    ...
    public static function post_meta_box_form($object, $box)
    {
        ...
        $ratingFormToShow         = get_post_meta($postId, '_cbrating_listing_ratingForm', true); //currently stored for the post
        ...
        if (!empty($ratingForms)) {
            foreach ($ratingForms as $ratingForm) {
                ...
                if ($ratingFormToShow == $ratingForm->id) {
                    $option .= '<option selected value="' . $ratingForm->id . '">' . $ratingForm->name . $txt . '(' . $ratingForm->id . ')</option>';
                } else {
                    $option .= '<option value="' . $ratingForm->id . '">' . $ratingForm->name . $txt . '(' . $ratingForm->id . ')</option>';
                }
            }
        }
        $option .= '</select>';
        ...
        <p>
            ...
            <input type="checkbox" id="enable_ratingForm" name="enable_ratingForm"
                   value="1" <?php echo((($ratingFormEnabled != '')) ? (($ratingFormEnabled == 1) ? 'checked' : '') : ('checked')); ?>/>
        </p>
        <p>
            <label for="is_default_ratingForm"><?php _e(sprintf("Use Default"), 'cbratingsystem'); ?></label>
            <input type="checkbox" id="is_default_ratingForm" name="is_default_ratingForm"
                   value="1" <?php echo((($defaultRatingFormEnabled != '')) ? (($defaultRatingFormEnabled == 1) ? 'checked' : '') : ('checked')); ?>/>
        </p>
        <p class="cb_listing_ratingForm">
            <label
                for="listing_ratingForm"><?php _e(sprintf("Select which form should work for this %s:", $object->post_type), 'cbratingsystem'); ?></label>
            <br/>
            <?php echo $option; ?>
            <span
                class="description"><?php _e(sprintf("Select which form should work for this %s. You can checkout rating forms at %s page.", $object->post_type, '<a target="_blank" href="' . $adminUrl . '">this</a>'), 'cbratingsystem'); ?></span>
        </p>
    ...
    public static function ratingForm_add_meta_data_filter($ratingFormID)
    {
        ...
        if (!empty($postId)) {
            ...
            //if custom rating form selected than the default one
            if (!empty($ratingFormToShow) && is_numeric($ratingFormToShow)) {
                //if not enabled then it should be '0', if enabled default empty for previously created post, and 1
                //if the post is asved
                if ($ratingFormEnabled != '0') {
