...
class CBRatingSystemAdmin
{
    ...
    public static function admin_load_scripts_and_styles($hook)
    {
        ...
        $plugin_page_slugs = array(
            ...
            'rating-review_page_ratingform',
            'rating-review_page_ratingformedit',
            'rating-review_page_rating_reports',
            'rating-review_page_rating_avg_reports',
            'rating-review_page_ratingform_theme',
            'rating-review_page_rating_tool'
        );

        if (!in_array($hook, $plugin_page_slugs)) return;


        wp_register_style('jquery-tablesorter-css', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'css/jquery.tablesorter.pager.css', array('cbratingsystem_admin_style'), CB_RATINGSYSTEM_PLUGIN_VERSION);
        wp_register_style('cbratingsystem_admin_style_tooltipster', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/tooltipster/css/tooltipster.css', array(), CB_RATINGSYSTEM_PLUGIN_VERSION);

        wp_register_style('cbratingsystem_admin_style', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'css/cbratingsystem.admin.css', array(), CB_RATINGSYSTEM_PLUGIN_VERSION);


        wp_enqueue_script('jquery');

        wp_register_script('cbratingsystem_admin_inline_edit_script', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/jquery.inline-edit.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);
        wp_register_script('jquery-tablesorter-js', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/jquery.tablesorter.min.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);
        wp_register_script('jquery-tablesorter-pager', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/jquery.tablesorter.pager.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);

        //wp_enqueue_script( 'jquery-ui-core' );
        //wp_enqueue_script( 'jquery-ui-tabs' );

        wp_register_script('cbratingsystem-tooltipster', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/tooltipster/js/jquery.tooltipster.min.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);

        wp_register_script('cbratingsystem-admin', CB_RATINGSYSTEM_PLUGIN_DIR_URL . 'js/cbratingsystem.admin.js', array('jquery'), CB_RATINGSYSTEM_PLUGIN_VERSION);


        wp_localize_script(
            'cbratingsystem-admin',
            'commentAjax',
            array(
                'ajaxUrl'      => admin_url('admin-ajax.php'),
                'sampleqtitle' => __('Sample Question Title', 'cbratingsystem'),
                'clicktoedit'  => __('Click to edit', 'cbratingsystem')
            )
        );


        wp_enqueue_style('jquery-tablesorter-css');
        wp_enqueue_style('cbratingsystem_admin_style_tooltipster');
        wp_enqueue_style('cbratingsystem_admin_style');


        wp_enqueue_script('cbratingsystem_admin_inline_edit_script');
        wp_enqueue_script('jquery-tablesorter-js');
        wp_enqueue_script('jquery-tablesorter-pager');
        wp_enqueue_script('cbratingsystem-tooltipster');
        wp_enqueue_script('cbratingsystem-admin');
    ...
    public static function admin_ratingForm_listing_page()
    {
        ...
        <div class="wrap">
            ...
            <h2><?php _e('CBX Rating & Review: Form Listing', 'cbratingsystem') ?></h2>
    ...
    public static function admin_ratingForm_listing_page_add()
    {
        ...
        <div class="wrap">
            ...
            <div id="icon-options-general" class="icon32"></div>
            <h2><?php _e('CBX Rating & Review: Form Edit /Add', 'cbratingsystem') ?></h2>
    ...
    {
        CBRatingSystemAdminReport::logReportPageOutput();
    }

    /**
     * User Rating average logs
     */
    public static function admin_rating_average_reports_page()
    {
        CBratinglogreportoutput::averageReportPageOutput();
    }
                        ...
                        if (!empty($firstLevelArray['field']) && !empty($firstLevelArray['field']['type'])) {
                            ...
                            if (in_array($type, $fieldWithArr)) {
                                ...
                                foreach ($firstLevelArray['field'][$type] as $fieldId => $field) {
                                    ...
                                    //var_dump('Field id ='.$fieldId.' field='.$field);
...
//			echo '</pre>';
        ...
        foreach ($form_default as $key => $item) {
            ...
            $tooltip      = ($tooltip == '' && isset($item['desc']) && $item['desc'] != '') ? $item['desc'] : '';
            $tooltip_html = ($tooltip != '') ? ' class="cbratingtooltipster" title="' . $tooltip . '" ' : '';
            ...
            if ($item['type'] == 'text') {
                ...
                $output .= '<tr>
									<th scope="row"><label ' . $tooltip_html . ' for="cbratingf_' . $key . '">' . $item['label'] . '</label></th>
                ...
                $radio_html = '';
                $radio_html .= '<tr>
											<th scope="row" ' . $tooltip_html . '>' . $item['label'] . '</th>
											<td>
												<fieldset>
                ...
                $selectoutput = '<tr valign="top">
											<th scope="row"><label for="cbratingf_' . $key . '" ' . $tooltip_html . '>' . $item['label'] . '</label></th>
											<td>
...
//					}
    ...
    public static function admin_ratingForm_listing_html()
    {
        ...
        $adminUrlSort = admin_url('admin.php?page=ratingform');

        //$sort			= isset($_GET['sort'])? esc_attr($_GET['sort']) : 'desc';
        ...
        //check if rating form delete action is submitted
        ...
        echo '<br/>';
        if ($add_more_cbform) {
            echo '<a id="button-addrating" class="button button-primary button-large button-addrating" href="' . admin_url('admin.php?page=ratingformedit') . '&id=0">' . __('Add New Form', 'cbratingsystem') . '</a>';
            ...
            echo '<p>' . sprintf(__('Note: <a href="%s" class="button" target="_blank">Upgrade</a> to Premium Version to add more forms', 'cbratingsystem'), 'http://codeboxr.com/product/multi-criteria-flexible-rating-system-for-wordpress') . '</p>';
        }
        ?>
                        ...
                        <tr>
                            <th id="cb" class="manage-column column-cb check-column" style="">
                                <label class="screen-reader-text"
                                       for="cb-select-all"><?php _e('Select All', 'cbratingsystem') ?></label>
                                <input id="cb-select-all" type="checkbox" style="">
                            </th>
                            <th width="3%" class="manage-column column-id ">
                                <?php _e('ID', 'cbratingsystem'); ?>
                            </th>
                            <th width="20%" class="manage-column column-title ">
                                <?php _e('Title/Name', 'cbratingsystem'); ?>
                            </th>
                            <th width="10%" class="manage-column column-status ">
                                <?php _e('Status', 'cbratingsystem'); ?>
                            </th>
                            <th width="30%" class="manage-column column-shortatg">
                                <?php _e('Shortcode Tag', 'cbratingsystem'); ?>
                            </th>
                            <th width="30%"
                                class="manage-column column-criteria"><?php _e('Criteria(s)', 'cbratingsystem'); ?></th>
                            <th width="12%"
                                class="manage-column column-action"><?php _e('Action', 'cbratingsystem'); ?></th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if ($ratingForms) : ?>
                            <?php foreach ($ratingForms as $k => $ratingForm) :
                                ...
                                $criteriaHtml  = '';
                                $criteriaHtml  = ($criteriaHtml) ? $criteriaHtml : __('No Criteria', 'cbratingsystem');
...
//															echo '</pre>';
                                ...
                                <tr class="<?php echo $oddEvenClass; ?> status-publish format-standard hentry category-uncategorized alternate iedit author-self">
                                    <th scope="row" class="check-column">
                                        <label class="screen-reader-text"
                                               for="cb-select-<?php echo $ratingForm->id; ?>"><?php echo __('Select Sample', 'cbratingsystem'); ?></label>
                                        <input id="cb-select-<?php echo $ratingForm->id; ?>" type="checkbox"
                                               value="<?php echo $ratingForm->id; ?>" name="delete_forms[]">
                                    </th>
                                    <td><?php echo $ratingForm->id; ?></td>
                                    <td><?php echo CBRatingSystemFunctions ::cb_l($title, ($adminUrl . $ratingForm->id)); ?></td>
                                    <td>
                                                        <span data-form-status="<?php echo $ratingForm->is_active; ?>"
                                                              data-form-id="<?php echo $ratingForm->id; ?>"
                                    ...
                                    <td>
                                        <?php if (get_option('cbratingsystem_defaultratingForm') != $ratingForm->id): ?>
                                            <button data-form-id="<?php echo $ratingForm->id; ?>" type="submit"
                                                    id="edit-submit" class="button button-primary button-large"
                                                    name="make_form_default" value="<?php echo $ratingForm->id; ?>">Make
                                                Default
                                            </button>
                                        <?php else: ?>
                                            <button data-form-id="<?php echo $ratingForm->id; ?>" disabled id=""
                                                    class="button-primary button-large button">Default
                                            </button>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else : echo '<td colspan="7" align="center">'.__('No Records Found','cbratingsystem').'</td>'; ?>
                        <?php endif; ?>
                        </tbody>
                        <tfoot>
                        <tr id="rtp-pager-logs" style="text-align: center;">
                            <td colspan="10">
                                <img class="first" title="Go To First"
                                     src="<?php echo CB_RATINGSYSTEM_PLUGIN_DIR_IMG; ?>arrow-stop-180.png"/>
                                <img class="prev" title="Previous"
                                     src="<?php echo CB_RATINGSYSTEM_PLUGIN_DIR_IMG; ?>arrow-180.png"/>
                                <input class="pagedisplay" type="text" readonly="true"/>
                                <img class="next" title="Next"
                                     src="<?php echo CB_RATINGSYSTEM_PLUGIN_DIR_IMG; ?>arrow.png"/>
                                <img class="last" title="Go To Last"
                                     src="<?php echo CB_RATINGSYSTEM_PLUGIN_DIR_IMG; ?>arrow-stop.png"/>
                    ...
                    <table class="widefat">
                        <tr>
                            ...
                            <td>
                                ...
                                <input class="button-secondary" name="cbrp-delete-form" type="submit"
                                       value="Delete Form"/>
    ...
    public static function cbAdminRatingFormListingAjaxFunction()
    {
        ...
        if (isset($_POST['cbRatingFormEnableData']) && !empty($_POST['cbRatingFormEnableData'])) {
