...
class CBRatingSystemFront
{
    ...
    public static function add_ratingForm_to_content($ratingFormArray)
    {
        ...
        //$table_name1    = $userlog_table;
        //$table_name2    = $form_table;
        ...
        $ratethis_label = (empty($isUserSubmittedRating))? __('Rate this','cbratingsystem'): __('Change your rating', 'cbratingsystem');

        $output = '<div class="cbrp_front_content">';
            $output .= '<h3  class="cbratingfrom_title">' . __('Ratings', 'cbratingsystem') . ' </h3>';
                                            ...
                                            foreach ($perCriteriaAverageRating as $cId => $criteria) {
                                                ...
                                                foreach ($stars as $star) {
                                                    if (isset($star['enabled']) && intval($star['enabled'])) {
                                                        $star_arr[] = __($star['title'], 'cbratingsystem');
                                                    }
                                                }
        ...
        $summary_output .= '<div class="clear" style="clear:both"></div>
                            <div  class="rating-average-label-form-' . $ratingFormArray['id'] . '-postid-' . $post_id . ' readonly-criteria-average-label readonly_criteria_average_label_form_' . $theme_key . '_theme -form rating-average-label-form-' . $ratingFormArray['id'] . '" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
                                ...
                                <span class="rating" itemprop="ratingValue">' . (number_format(($perPostAverageRating / 100) * 5, 2)) . '</span> ' . __('out of', 'cbratingsystem') . '
                                <span itemprop="bestRating">5</span> ' . __('with', 'cbratingsystem') . '
                                <span  class="total_rates">  ' . __('based on', 'cbratingsystem') . ' <strong class="total_rates_count" itemprop="ratingCount">' . (!empty($avgRatingData[0]['per_post_rating_count']) ? (int)$avgRatingData[0]['per_post_rating_count'] : '0') . '</strong> ' . __('rating(s)', 'cbratingsystem') . ' </span>
                            </div>';
                ...
                foreach ($customPerCriteriaAverageRating['editor'] as $cId => $criteria) {
                    ...
                    $summary_output .= '
																			<div data-form-id="' . $ratingFormArray['id'] . '" data-label-id="' . $cId . '" class="readonly-criteria-wrapper readonly_criteria_wrapper_' . $theme_key . '_theme  readonly-criteria-id-wrapper-' . $cId . ' readonly-criteria-wrapper-form-' . $ratingFormArray['id'] . '">
																				...
																				<div data-hints="' . $star_arr . '" data-form-id="' . $ratingFormArray['id'] . '" data-label-id="' . $cId . '" class="editor-criteria-star-wrapper readonly-criteria-star-wrapper-id-' . $cId . '-form-' . $ratingFormArray['id'] . ' readonly-criteria-star-wrapper-id-' . $cId . ' criteria-star-wrapper-form-' . $ratingFormArray['id'] . '" id="criteria-star-wrapper">
        ...
        $summary_output .= '</div>'; //end of <div cbrp_switch_report_
            ...
            if($user_can_give_vote || $user_can_edit_vote){
                    ...
                    foreach ($ratingFormArray['custom_criteria'] as $firstLabel => $firstLabelArray) {
                        ...
                        $form_output .= '
                                                                                        <div data-form-id="' . $ratingFormArray['id'] . '" data-score-val="'.$score_value.'"  data-label-id="' . $firstLabel . '" class="criteria_wrapper_' . $theme_key . '_theme criteria-wrapper criteria-id-wrapper-' . $firstLabel . ' criteria-id-wrapper-' . $firstLabel . '-form-' . $ratingFormArray['id'] . ' criteria-wrapper-form-' . $ratingFormArray['id'] . '">
                                                                                            ...
                                                                                            <div data-form-id="' . $ratingFormArray['id'] . '" data-score-val="'.$score_value.'"  data-label-id="' . $firstLabel . '" class="criteria-star-wrapper criteria-star-wrapper-id-' . $firstLabel . ' criteria-star-wrapper-id-' . $firstLabel . '-form-' . $ratingFormArray['id'] . '" id="criteria-star-wrapper">
                ...
                if ($user_id == 0) {
                    $form_output .= '<div class="user_info">
                                                                <div class="user_name">
                                                                    <label for="user_name_field-' . $ratingFormArray['id'] . '">' . __('Name', 'cbratingsystem') . '</label>
                                                                ...
                                                                <div class="user_email">
                                                                    <label for="user_email_field-' . $ratingFormArray['id'] . '">' . __('Email', 'cbratingsystem') . '</label>
            ...
            /*
            $output .= '<input type="hidden" name="rp_id" value="' . $ratingFormArray['id'] . '-' . $post_id . '" />
                        <input type="hidden" name="cbrnonce" value="' . wp_create_nonce('cb_ratingForm_front_form_nonce_field') . '" />
                        <input type="hidden" name="formId" value="ratingForm" />';
            */
        ...
        if (isset($_POST['cbRatingData']) && !empty($_POST['cbRatingData'])) {
            ...
            if (wp_verify_nonce($returnedData['cbrp_nonce'], 'cb_ratingForm_front_form_nonce_field')) {
                if (!empty($returnedData['values'])) {
                    ...
                    //$sql_get_loggin = "SELECT logging_method FROM $form_table where id=" . $insertArray['form_id'];
                    ...
                    if ($user_id > 0) {
                        //if logged in user
                        //$sql   = $wpdb->prepare("SELECT COUNT(ur.id) AS count FROM $table_name1 ur WHERE ur.form_id=%d AND ur.post_id=%d AND ur.user_id=%d", $insertArray['form_id'], $insertArray['post_id'], $user_id);
                        ...
                        if (in_array("cookie", $rating_Logmethod) && !in_array("ip", $rating_Logmethod)) {
                            //$sql   = $wpdb->prepare("SELECT COUNT(ur.id) AS count FROM $table_name1 ur WHERE ur.form_id=%d AND ur.post_id=%d AND ur.user_session = %s", $insertArray['form_id'], $insertArray['post_id'], $user_session);
                            ...
                            //$sql   = $wpdb->prepare("SELECT COUNT(ur.id) AS count FROM $table_name1 ur WHERE ur.form_id=%d AND ur.post_id=%d AND ur.user_ip = %s", $insertArray['form_id'], $insertArray['post_id'], $user_ip);
                            ...
                            //$sql   = $wpdb->prepare("SELECT COUNT(ur.id) AS count FROM $table_name1 ur WHERE ur.form_id=%d AND ur.post_id=%d AND ur.user_ip = %s AND ur.user_session = %s", $insertArray['form_id'], $insertArray['post_id'], $user_ip, $user_session);
                    ...
                    if ($user_allowed_vote || $user_allowed_revote) {
                            ...
                            if (!empty($returnedData['user_email'])) {
                                ...
                                if (is_email(trim($returnedData['user_email']))) {
                                    if (!email_exists(trim($returnedData['user_email']))) {
                                        ...
                                        $email_valid     = false;
                                        $email_error_msg = __('Sorry, this email address is associated with a registered user and can not be use for guest voting.', 'cbratingsystem');
                                    }
                                } else {
                                    $email_valid     = false;
                                    $email_error_msg = __('Invalid Email addresss', 'cbratingsystem');
                                }

                            } else {
                                $email_valid     = false;
                                $email_error_msg = __('Email field can\'t be empty', 'cbratingsystem');
                            }
                        ...
                        //do_action('cbratingsystem_before_comment_posted' , $insertArray);
                ...
                foreach ($reviews as $reviewKey => $review) {
                    ...
                    if (!empty($review->rating) && is_array($review->rating)) {
                        ...
                        $mainContent .= '  <div class="reviews_user_details_' . $theme_key . '_theme review_user_details">
                                            			' . $user_html . '
                                            			<span class="user_rate_time"><a title="' . cbratingsystem_date_display($review->created) . '" href="' . get_permalink($post_id) . '#cbrating-' . $ratingFormId . '-review-' . $review->id . '">' . CBRatingSystemFunctions :: codeboxr_time_elapsed_string($review->created) . '</a></span>
                                        			</div>
                                        			<div class="clear" style="clear:both;"></div>
                        		';
                        */
                        ...
                        */
                        ...
                        if (!empty($review->comment) && is_string($review->comment)) {
                            ...
                            $comment_output = '<p class="comment" itemprop="reviewBody">' . stripslashes($review->comment) . '</p>';
                            ...
                            if ($reviewOptions['comment_status'] != 'approved') {
                                ...
                                if ($reviewOptions['comment_status'] == 'unverified') {
                                    ...
                                    $review_status = sprintf(__('Your comment is %s [please check your mail to verify]', 'cbratingsystem'), $reviewOptions['comment_status']);
                                } else {
                                ...
                                $mainContent .= '<div class="review_user_rating_comment_' . $theme_key . '_theme review_user_rating_comment">
		                                            	        <strong>' . __('Comment', 'cbratingsystem') . ': </strong> ' . $comment_output . ' (' . $review_status . ')' . '
		                                        	          </div>
		                                        	          <div class="clear" style="clear:both;"></div>
		                                     ';
                            } else {
                                $mainContent .= '<div class="review_user_rating_comment_' . $theme_key . '_theme review_user_rating_comment">
		                                            	            <strong>' . __('Comment', 'cbratingsystem') . ':</strong> ' . $comment_output . '
		                                        	          </div>
		                                        	         <div class="clear" style="clear:both;"></div>
		                                     ';
